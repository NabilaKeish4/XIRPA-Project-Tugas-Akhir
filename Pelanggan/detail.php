...
<head>
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($produk['nama_produk']) ?> - PlantHub</title>
...
<body class="bg-emerald-50 text-gray-800">
    <div class="container mx-auto p-6 max-w-4xl">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
            <img src="../assets/img/<?= htmlspecialchars($produk['foto_produk'] ?? 'default.jpg') ?>" class="w-full h-80 object-cover rounded-lg">
            
            <div class="flex flex-col justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-emerald-900"><?= htmlspecialchars($produk['nama_produk']) ?></h1>
                    <p class="text-2xl font-semibold text-emerald-600 my-3">Rp <?= number_format($produk['harga'], 0, ',', '.') ?></p>
                    <p class="text-sm text-gray-600 mb-4">Stok Tersedia: <span class="font-bold"><?= (int)$produk['stok'] ?></span></p>
                    
                    <h3 class="font-semibold text-gray-700 border-b pb-1 mb-2">Deskripsi Produk:</h3>
                    <p class="text-gray-600 text-sm mb-4"><?= nl2br(htmlspecialchars($produk['deskripsi'])) ?></p>

                    <h3 class="font-semibold text-gray-700 border-b pb-1 mb-2">Tips Perawatan 💧:</h3>
                    <p class="text-gray-600 text-sm mb-4"><?= nl2br(htmlspecialchars($produk['cara_perawatan'] ?? 'Siram 1-2 kali sehari dan beri pencahayaan cukup.')) ?></p>
                </div>

                <form action="dashboard.php" method="POST" class="flex gap-2">
                    <input type="hidden" name="id_produk" value="<?= (int)$produk['id_produk'] ?>">
                    <input type="number" name="jumlah" value="1" min="1" max="<?= (int)$produk['stok'] ?>" class="w-20 border rounded-lg p-2 text-center">
