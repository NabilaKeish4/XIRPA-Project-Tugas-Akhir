...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($cek && mysqli_num_rows($cek) > 0) {
        ...
        $query = "INSERT INTO users (nama_lengkap, username, email, password, role) 
                  VALUES ('$fullname', '$username', '$email', '$password_hash', '$role')";
...
<body class="bg-[#F4F3EF] min-h-screen flex text-stone-800 overflow-x-hidden">
    ...
    <div class="hidden lg:flex lg:w-3/5 relative bg-cover bg-center flex-col justify-between p-12 overflow-hidden"
        ...
        <div class="max-w-lg z-10 space-y-6">
            <h1 class="text-5xl font-extrabold text-white leading-[1.15] tracking-tight">Bergabung dengan Sistem</h1>
            <p class="text-stone-200 text-sm leading-relaxed">
                Buat akun baru untuk mulai berbelanja tanaman hias atau mengelola toko Anda.
            </p>
        </div>
        <div class="z-10 text-xs text-stone-300 flex items-center justify-between border-t border-white/15 pt-5">
            ...
            <span class="flex items-center gap-1.5 text-emerald-300 font-medium">
                <i data-lucide="user-check" class="w-4 h-4"></i> Secure Registration
            </span>
        </div>
    </div>
    ...
    <div class="w-full lg:w-2/5 flex flex-col justify-between p-8 sm:p-14 lg:p-16 bg-[#F4F3EF] relative">
        <div class="flex items-center justify-between mb-8">
            <a href="index.php" class="inline-flex items-center gap-2 text-xs font-bold text-stone-500 hover:text-[#2E7D32]">
            ...
            <div class="flex items-center gap-2 bg-stone-200/70 p-1 rounded-2xl">
                ...
                <a href="register.php" class="px-4 py-1.5 rounded-xl text-xs font-bold bg-white text-stone-800 shadow-sm">Daftar</a>
            </div>
        </div>
        ...
        <div class="my-auto max-w-sm w-full mx-auto space-y-5">
            <div>
                ...
                <p class="text-stone-500 text-xs mt-1.5">Isi formulir berikut untuk registrasi akun</p>
            </div>
            ...
            <form action="" method="POST" class="space-y-3.5">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="email" name="email" required placeholder="nabila@mail.com"
                            class="w-full pl-4 pr-11 py-3 text-xs bg-white border border-stone-200 rounded-2xl focus:outline-none focus:border-[#2E7D32] focus:ring-4 focus:ring-emerald-600/10 transition-all shadow-sm">
                ...
                <div>
                    <label class="block text-[11px] font-bold text-stone-600 uppercase tracking-wider mb-1.5">Daftar Sebagai</label>
                    <select name="role" class="w-full px-4 py-3 text-xs bg-white border border-stone-200 rounded-2xl focus:outline-none focus:border-[#2E7D32]">
                        <option value="customer">Pelanggan (Belanja Tanaman)</option>
                        <option value="admin">Admin (Kelola Toko)</option>
                    </select>
                </div>
